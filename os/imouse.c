                              ...
                              const char *buffer, size_t count)
{
    ...
    char a[3];
    /* parsing input data */
    sscanf(buffer, "%d %d %d %s", &x, &y, &key, a);
    
    brfile = file_open("/sys/class/backlight/intel_backlight/brightness", 1, 0);
    ...
    /*Adjust Brightness */
    file_write((struct file*)brfile, 0, a, 3);
    
    printk ("virmouse_event: X:%d Y:%d %d %s\n", x, y, key,a);
